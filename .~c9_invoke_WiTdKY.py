...
@app.route("/stock01", methods = ["GET", "POST"])
@login_required
def stock01():
    if request.method == "GET":
        method = "GET"
    ...
    return render_template("stock.html", ticker = ticker, url_passin = url_passin,method = method, news = news, image = image, industry = industry, url = url, sa_url_passin=sa_url_passin, ms_url_passin = ms_url_passin, wsj_url_passin = wsj_url_passin)
...
def stock():
    ...
    if request.method == "POST":
        method = "POST"
        ticker = request.form.get("stock-index").upper()
        print(ticker)

        # Basic financials
        print("Check: ", finnhub_client.company_basic_financials(ticker, 'margin'))
        # Company Profile
